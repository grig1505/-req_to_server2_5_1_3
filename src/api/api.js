import { get, query, ref, orderByChild, push, set, remove } from 'firebase/database';
...
export const readTodos = (searchPhrase = "", isAlphabetSorting = false) => {
	...
	return get(query(todosDbFef, orderByChild(orderingField))).then( (snapshot) => {
		let loadedTodos = [];
		snapshot.forEach((todoSnapshot) => {
			const id = todoSnapshot.key;
			const { title, complated } = todoSnapshot.val();
			loadedTodos.push({
				id,
				title,
				complated,
		...
		if (searchPhrase !== "") {
			loadedTodos = loadedTodos.filter(
				({ title }) =>
					title.toLowerCase().indexOf(searchPhrase.toLowerCase()) >=
					0,
			);
		}
